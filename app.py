...
# 📱 프리미엄 모바일 레이아웃 설정
st.set_page_config(page_title="솔 운명상점 Lite Premium V14", layout="centered", initial_sidebar_state="collapsed")
...
h3 { display: none; } /* AI가 만드는 파트제목 숨김 (아코디언 제목으로 쓰임) */
    ...
    if not name1 or not birth1:
        ...
        with st.spinner("AI 마스터가 심층 데이터 표를 포함한 보고서를 구성 중입니다... (최대 1분 소요)"):
            ...
            if mode == "👤 개인 사주 리포트":
                ...
                [작성 핵심 지침 - 매우 중요]
                1. 이모티콘 100% 강제: 이전에 이모티콘이 누락되는 문제가 있었다. **모든 표의 항목과 내용에는 반드시 직관적이고 다채로운 이모티콘을 듬뿍 사용하여** 프리미엄 시각적 효과를 극대화하라.
                2. 한글화 및 용어 순화: 영어를 전혀 쓰지 말고 '타고난 핵심 명식, 마음가짐, 실천 지침' 등 품격 있는 한글만 사용하라. '용신'은 '나를 돕는 수호 기운'으로 풀어서 적어라.
                3. 제1장 오행 개수 부활: 1장 '타고난 핵심 명식' 표에는 반드시 오행의 분포 개수(예: 🔥화3, 💧수2, 🌳목1, 🪨토2, ⚔️금0)를 정확히 세어서 포함하라.
                4. 제2장 모든 신살 강도별 나열: 명식에 있는 특수신살을 2개만 적지 말고, **보유한 모든 신살(백호, 괴강, 도화 등)을 빠짐없이 찾아 표에 넣고 '강도 (높음/보통/낮음)' 순으로 나열**하라.
                5. 제8장 운세표 행동 강령 압축: 26-27년 운세 기상도의 표는 `| 시기 | 핵심 흐름 | 행동 |` 3개 열로 구성하라. 특히 **'행동' 칸에는 절대 긴 문장을 쓰지 말고, 반드시 `🚀 도전`, `🎯 집중`, `🔍 관망`, `🛡️ 유지` 처럼 [이모티콘 + 2~4글자 단어] 형태**로만 요약하여 모바일 가독성을 높여라.
                6. 요약(>) 블록 완전 삭제: 파트 제목 밑의 불필요한 요약 줄 없이 바로 마크다운 표부터 시작하라.

                [필수 구성 10파트] (모든 파트는 '표 -> 서술 1~2문단' 순서)
                1. ✨ 타고난 핵심 명식 (오행 개수 필수 포함 표 -> 서술)
                2. 💫 특수 기운과 신살 (보유한 모든 신살을 강도별로 표기하고 현대적으로 재해석한 표 -> 서술)
                3. 🌗 성격의 명암 (강점/약점 키워드 대비표 -> 서술)
                4. 💎 재물운의 그릇 (재물 시기/형태 표 -> 서술)
                5. 🚀 성공의 자리 (추천 직무/분야 표 -> 서술)
                6. 🤝 인복과 귀인 (귀인/악연 구별표 -> 서술)
                7. 🧘 건강과 활력 (주의 장기/건강 관리표 -> 서술)
                8. 🌤️ 2026-2027 운세 기상도 (시기, 핵심 흐름, 행동 3열로 구성. 행동은 단어만 -> 서술)
                ...
                [작성 핵심 지침 - 매우 중요]
                1. 이모티콘 100% 강제: **모든 표의 항목과 내용에 직관적인 이모티콘을 적극적으로 사용하여** 시각적 완성도를 극대화하라.
                2. 한글화 및 용어 순화: 영어를 쓰지 말고 '타고난 핵심 명식, 마음가짐, 대인 관계' 등 품격 있는 100% 한글 명칭을 사용하라. 
                3. 제7장 운세표 행동 강령 압축: 운세 기상도의 표는 `| 시기 | 핵심 흐름 | 행동 |` 3개 열로 구성하라. **'행동' 칸에는 절대 문장을 쓰지 말고, 반드시 `🚀 도전`, `🎯 집중`, `🔍 관망`, `🤝 양보` 처럼 [이모티콘 + 2~4글자 단어]**로만 요약하여 모바일 가독성을 확보하라.
                4. 요약(>) 블록 완전 삭제: 불필요한 요약 줄 없이 곧바로 마크다운 표부터 시작하라.
                5. 선표/후서술 원칙: 8개의 모든 파트에 반드시 두 사람을 비교/분석하는 표를 먼저 띄우고 그 아래에 1~2문단의 서술을 작성하라.
                ...
                7. 📈 2026-2027 운세 기상도 (시기, 핵심 흐름, 행동 3열. 행동은 단어만 -> 서술)
            ...
            # 🔘 오직 '폰에 리포트 저장하기' 버튼 하나만 직관적으로 제공
            st.download_button(
                label="📥 폰에 리포트 저장하기", 
                data=final_html, 
