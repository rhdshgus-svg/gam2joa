...
# 📱 프리미엄 모바일 레이아웃 설정
st.set_page_config(page_title="솔 운명상점 Lite Premium V20", layout="centered", initial_sidebar_state="collapsed")
    ...
    if not name1 or not birth1:
        ...
        with st.spinner("AI 마스터가 심층 데이터 표를 포함한 보고서를 구성 중입니다... (최대 1분 소요)"):
            ...
            if mode == "👤 개인 사주 리포트":
                ...
                3. 파트 분리 엄수: 반드시 아래 제공된 1~10 파트의 템플릿을 그대로 복사해서 사용하라.
            ...
            for p in parts:
                if p.strip():
                    lines = p.strip().split("\n", 1)
                    # 💡 강제 예방: 제목 앞의 필요없는 기호 완벽 차단. 
                    # 만약 AI가 헛소리를 앞에 붙였더라도 아코디언에는 제목만 깔끔하게 나옵니다.
                    title = re.sub(r'^[\d\.\s\*#]+', '', lines[0]).strip() 
                    
                    if len(lines) > 1 and title:
                        content = markdown.markdown(lines[1].strip(), extensions=['tables'])
                        chapters_html += f"""
                        <details>
