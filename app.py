...
# 📱 프리미엄 모바일 레이아웃 설정
st.set_page_config(page_title="솔 운명상점 Lite Premium V17", layout="centered", initial_sidebar_state="collapsed")
    ...
    if not name1 or not birth1:
        ...
        with st.spinner("AI 마스터가 심층 데이터 표를 포함한 보고서를 구성 중입니다... (최대 1분 소요)"):
            ...
            if mode == "👤 개인 사주 리포트":
                ...
                [작성 핵심 지침 - 매우 중요]
                1. 표 열(Column) 임의 추가 절대 금지: 표 안에 '아이콘', '기호' 같은 열을 임의로 절대 만들지 마라. 표는 반드시 제시된 형식(`| 구분 | 분석 내용 |`)만 준수해야 한다.
                2. 키워드 압축 및 자연스러운 이모티콘: 표 안의 내용은 긴 문장을 빼고 핵심 키워드로 압축하라. 가독성을 높일 필요가 있는 부분에만 자연스럽게 이모티콘을 곁들여라 (무분별한 도배 금지).
                3. 기호 사용 금지: 파트 제목에 마크다운 강조 기호인 ** 를 절대 사용하지 마라.
                4. 한글화 및 용어 순화: 영어를 쓰지 말고 100% 한글 명칭만 사용하라. '용신'은 '나를 돕는 수호 기운'으로 표기하라.
                5. 제1장 오행 개수 명시: 1장 '타고난 핵심 명식' 표에는 오행 분포(예: 🔥화3, 💧수2, 🌳목1, 🪨토2, ⚔️금0)를 정확히 세어 포함하라.
                6. 제2장 모든 신살 강도별 나열: 명식 내 숨은 신살(백호, 괴강, 도화 등)을 빠짐없이 찾아 '강도(높음/보통/낮음)'를 표기하고 현대적으로 재해석하라.
                7. 제8장 운세표 행동 강령 압축: 26-27년 운세 기상도의 표는 3열(`| 시기 | 핵심 흐름 | 행동 |`)로 구성하고, '행동' 칸에는 설명 없이 단어(예: 🚀 시작, 🎯 집중, 🔍 관망)만 적어라.
                8. 요약(>) 블록 완전 삭제: 파트 제목 아래에 바로 표(Table)부터 시작하라.
                ...
                8. 🌤️ 2026-2027 운세 기상도 (시기, 흐름, 행동 3열 표. 행동은 단어만 -> 서술)
                ...
                [작성 핵심 지침 - 매우 중요]
                1. 표 열(Column) 임의 추가 절대 금지: 표 안에 '아이콘', '기호' 같은 열을 임의로 절대 만들지 마라. 표는 반드시 제시된 형식(`| 구분 | {name1} | {name2} |`)만 준수해야 한다.
                2. 키워드 압축 및 자연스러운 이모티콘: 표 안의 내용은 긴 문장을 빼고 핵심 키워드로 압축하라. 가독성을 높일 필요가 있는 부분에만 자연스럽게 이모티콘을 곁들여라.
                3. 기호 사용 금지: 파트 제목에 마크다운 강조 기호인 ** 를 절대 사용하지 마라.
                4. 한글화 및 용어 순화: 영어를 쓰지 말고 품격 있는 100% 한글 명칭을 사용하라. 
                5. 제7장 운세표 행동 강령 압축: 운세 기상도의 표는 3열(`| 시기 | 핵심 흐름 | 행동 |`)로 구성하라. '행동' 칸에는 문장을 빼고 단어(예: 🚀 도전, 🎯 집중, 🔍 관망)만 적어라.
                6. 요약(>) 블록 완전 삭제: 파트 제목 아래에 곧바로 마크다운 표부터 띄워라.
                7. 선표/후서술 원칙: 8개의 모든 파트에 반드시 두 사람을 비교하는 표를 먼저 띄우고 서술하라.
            ...
            for p in parts:
                if p.strip():
                    lines = p.strip().split("\n", 1)
                    # 💡 핵심 방어 로직: AI가 붙인 별표(**)나 샵(#) 기호를 강제로 깨끗하게 지워버립니다.
