...
# 📱 프리미엄 모바일 레이아웃 설정
st.set_page_config(page_title="솔 운명상점 Lite Premium V16", layout="centered", initial_sidebar_state="collapsed")
    ...
    if not name1 or not birth1:
        ...
        with st.spinner("AI 마스터가 심층 데이터 표를 포함한 보고서를 구성 중입니다... (최대 1분 소요)"):
            ...
            if mode == "👤 개인 사주 리포트":
                ...
                [작성 핵심 지침 - 매우 중요]
                1. 시각적 기호로서의 이모티콘 활용: 모바일 표 안에는 긴 텍스트 대신 의미를 직관적으로 전달하는 이모티콘을 '아이콘'처럼 스마트하게 활용하라.
                2. 기호 사용 금지: 파트 제목에 마크다운 강조 기호인 ** 를 절대 사용하지 마라.
                3. 한글화 및 용어 순화: 영어를 쓰지 말고 100% 한글 명칭만 사용하라. '용신'은 '나를 돕는 수호 기운'으로 표기하라.
                4. 제1장 오행 개수 명시: 1장 '타고난 핵심 명식' 표에는 오행 분포(예: 🔥화3, 💧수2, 🌳목1, 🪨토2, ⚔️금0)를 정확히 세어 포함하라.
                5. 제2장 모든 신살 강도별 나열: 명식 내 숨은 신살(백호, 괴강, 도화 등)을 빠짐없이 찾아 '강도(높음/보통/낮음)'를 표기하고 현대적으로 재해석하라.
                6. 제8장 운세표 행동 강령 압축: 26-27년 운세 기상도의 표는 3열(`| 시기 | 핵심 흐름 | 행동 |`)로 구성하고, '행동' 칸에는 설명 없이 `🚀 시작`, `🎯 집중`, `🔍 관망`, `🛡️ 유지` 처럼 [이모티콘 + 2~4글자 단어]로만 요약하라.
                7. 요약(>) 블록 완전 삭제: 파트 제목 아래에 바로 표(Table)부터 시작하라.
                ...
                [작성 핵심 지침 - 매우 중요]
                1. 시각적 기호로서의 이모티콘 활용: 좁은 표 안의 텍스트를 대체할 수 있는 직관적인 이모티콘을 '아이콘'으로 활용하여 내용을 극도로 압축하라.
                2. 기호 사용 금지: 파트 제목에 마크다운 강조 기호인 ** 를 절대 사용하지 마라.
                3. 한글화 및 용어 순화: 영어를 쓰지 말고 품격 있는 100% 한글 명칭을 사용하라. 
                4. 제7장 운세표 행동 강령 압축: 운세 기상도의 표는 3열(`| 시기 | 핵심 흐름 | 행동 |`)로 구성하라. '행동' 칸에는 문장을 빼고 반드시 `🚀 도전`, `🎯 집중`, `🔍 관망`, `🤝 양보` 처럼 [이모티콘 + 2~4글자 단어]로만 요약하라.
                5. 요약(>) 블록 완전 삭제: 파트 제목 아래에 곧바로 마크다운 표부터 띄워라.
                6. 선표/후서술 원칙: 8개의 모든 파트에 반드시 두 사람을 비교하는 표를 먼저 띄우고 서술하라.
