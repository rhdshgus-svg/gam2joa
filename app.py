...
# 📱 프리미엄 모바일 레이아웃 설정
st.set_page_config(page_title="솔 운명상점 Lite Premium V19", layout="centered", initial_sidebar_state="collapsed")
    ...
    if not name1 or not birth1:
        ...
        with st.spinner("AI 마스터가 심층 데이터 표를 포함한 보고서를 구성 중입니다... (최대 1분 소요)"):
            ...
            if mode == "👤 개인 사주 리포트":
                ...
                [시스템 강제 지침 - 에러 방지용]
                1. 파트 분리 엄수: 반드시 아래 제공된 1~10 파트의 템플릿(### 1. 파트명)을 그대로 복사해서 사용하라. 제목 앞의 '###'가 누락되면 시스템이 붕괴된다.
                2. 임의 열 추가 금지: 표 안에 '아이콘', '기호' 같은 열을 함부로 신설하지 마라.
                3. 이모티콘 활용: 표 안의 텍스트가 길어지지 않도록 내용 중간중간 의미를 돕는 이모티콘을 추가하여 요약하라.
                4. 한글화: 영어 금지. 용신(수호 기운)은 모바일 가독성을 위해 '**필요 기운**'으로 짧게 표기하라.
                5. 신살 전체 나열: 제2장 표 작성 시, 명식에 존재하는 모든 신살(백호, 괴강, 도화, 화개, 역마, 귀문 등)을 찾아내어 행을 필요한 만큼 3개든 4개든 계속 추가해서 빠짐없이 모두 나열하라.

                [출력 템플릿] - 아래 양식을 그대로 사용하여 10파트 전체를 작성하라.
                ...
                [시스템 강제 지침 - 에러 방지용]
                1. 파트 분리 엄수: 반드시 아래 제공된 1~8 파트의 템플릿(### 1. 파트명)을 그대로 복사해서 사용하라. 제목 앞의 '###'가 누락되면 시스템이 붕괴된다.
                2. 임의 열 추가 금지: 표 안에 '아이콘' 등의 열을 신설하지 마라.
                3. 이모티콘 활용: 표 텍스트 중간에 이모티콘을 활용하여 내용을 요약하라.
                4. 한글화: 영어 절대 금지. 짧고 명확한 한글(예: 필요 기운)을 사용하라.
            ...
            for p in parts:
                if p.strip():
                    lines = p.strip().split("\n", 1)
                    # 💡 강제 예방: 제목의 별표(**), 샵(#), 숫자(1. ) 등을 모두 제거하여 깔끔하게 만듦
